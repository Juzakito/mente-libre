-- =================================================================================
-- MENTE LIBRE — MIGRATION 001: Initial Schema
-- =================================================================================
-- Creates the core tables, triggers, indexes, and RLS policies.
...
-- 2.1 Universities (B2B Multi-Tenant)
...
-- 2.2 Users (Extends auth.users from Supabase)
...
CREATE TABLE IF NOT EXISTS public.posts (
  ...
  author VARCHAR(255),
  avatar VARCHAR(50),
  text TEXT,  -- Alias: "content" for backwards compat
  content TEXT,
  tags TEXT[] DEFAULT '{}',
...
-- 2.7 Interventions (I-CARE Protocol)
CREATE TABLE IF NOT EXISTS public.interventions (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID REFERENCES public.users(id) ON DELETE CASCADE,
  university_id UUID REFERENCES public.universities(id),
  trigger_source VARCHAR(100),  -- 'chat_ai', 'mood_pattern', 'manual'
  risk_category VARCHAR(100),   -- 'low', 'medium', 'high', 'critical'
...
CREATE INDEX IF NOT EXISTS idx_moods_user ON public.moods(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_moods_university_date ON public.moods(university_id, created_at);
CREATE INDEX IF NOT EXISTS idx_posts_university_date ON public.posts(university_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_interventions_university ON public.interventions(university_id, status);
...
CREATE POLICY "Users can insert posts"
  ON public.posts FOR INSERT WITH CHECK (auth.uid() = user_id OR user_id IS NULL);
